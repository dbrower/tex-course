...
Most distributions include speciality editors which are integrated with LaTeX.
Two such are TeXshop and TeXstudio.


\section{Hello World}
For the rest of this course we will assume you are using ShareLaTeX.

Start by making a new project: choose the button ``New Project'' and then choose ``Blank Project''.
A new project is created and you should see a document with some text already in it.
Delete all of the text and type the following.
...
Click the ``Recompile'' button and you should see a page with the words ``Hello World!'' appear to the right.

The In this file, the backslash, \textbackslash, is used to tell LaTeX that the following word is a formatting command.
...
The next commands are the pair \verb=\begin{document}= and \verb=\end{document}=.
These commands enclose the text we wish to be typeset, and the pair always appear in a LaTeX file.
The text between the \verb=\documentclass= and the \verb=\begin{document}= is called the \emph{preamble}.
The preamble is used to load additional formatting packages we wish to use, or to define
custom commands for use in our document.

% makes a bunch of files: hello.log, hello.aux, hello.dvi, hello.blg, hello.bbl, hello.pdf, hello.synctex.gz

Should you with to typeset an actual backslash character, you can use the command
\verb=\textbackslash=.
...
\end{center}
We will go over their meanings later.
...
\begin{itemize}
    ...
    \item If LaTeX gets the hyphenation of a word wrong, you can tell LaTeX the correct hyphenation with the \verb=\hyphenation{...}= command.
        ...
        (Confession: I have never needed this command.)

    \item Force a page break with the command \verb=\pagebreak=
    ...
    \item A common reason for horizontal space is to leave blank space for worksheets.
        Sometimes a line is desired for those. In LaTeX that line is called a \textit{rule}: \verb=Name: \rule[-.5ex]{2in}{.1pt}=, Name: \rule[-.5ex]{2in}{.1pt}.
\end{itemize}
...
There are websites that allow one to work on LaTeX documents collaboratively, much like Google Docs.
They also keep your files on their servers, so you can access them from any computer.
I think both offer free and paid plans.
I haven't used either, and would welcome any feedback on which is better or on other services I haven't listed.
...
\begin{itemize}
    ...
    \item Leslie Lamport, ``LATEX: a document preparation system''
        ...
        Notre Dame has copies in the library, \url{http://onesearch.library.nd.edu/NDU:malc_blended:ndu_aleph001563762}
